...
import 'package:naturafit/services/trainer_services/edit_client_bloc.dart';
...
class EditClientPage extends StatefulWidget {
  ...
  final Map<String, dynamic>? clientInfoEnteredByTrainer;
  final bool isAppUser;
  const EditClientPage({
    Key? key, 
    required this.client,
    this.clientInfoEnteredByTrainer,
...
class _EditClientPageState extends State<EditClientPage> {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (context) => EditClientBloc()),
      ],
      child: _EditClientPageContent(
        client: widget.client,
        clientInfoEnteredByTrainer: widget.clientInfoEnteredByTrainer,
...
class _EditClientPageContent extends StatefulWidget {
  final Map<String, dynamic> client;
  final Map<String, dynamic>? clientInfoEnteredByTrainer;
  final bool isAppUser;
  const _EditClientPageContent({
    Key? key, 
    required this.client,
    this.clientInfoEnteredByTrainer,
    required this.isAppUser,
  ...
  @override
  State<_EditClientPageContent> createState() => _EditClientPageContentState();
}

class _EditClientPageContentState extends State<_EditClientPageContent>
  ...
  void initState() {
    ...
    // Initialize data based on whether it's an app user or manually added client
    final clientData = widget.isAppUser 
        ? widget.clientInfoEnteredByTrainer ?? {}
        : widget.client;

    // Populate form fields with existing data
    _nameController.text = clientData['clientName'] ?? clientData['name'] ?? '';
    _emailController.text = clientData['email'] ?? '';
    _phoneController.text = clientData['phone'] ?? '';
    
    // Handle birthday
    if (clientData['birthday'] != null) {
      DateTime? birthDate;
      if (clientData['birthday'] is Timestamp) {
        birthDate = clientData['birthday'].toDate();
      } else if (clientData['birthday'] is String) {
        try {
          final parts = clientData['birthday'].split('-');
          birthDate = DateTime(
      ...
      if (birthDate != null) {
        ...
        _birthdayController.text = '${birthDate.year}-${birthDate.month.toString().padLeft(2, '0')}-${birthDate.day.toString().padLeft(2, '0')}';
      }
    }

    // Set gender
    _selectedGender = clientData['gender'] ?? 'Not Specified';
    ...
    // Set measurements
    _selectedHeight = (clientData['height'] ?? 170.0).toDouble();
    _displayHeight = myHeightUnit == 'cm' ? _selectedHeight : unitPreferences.cmToft(_selectedHeight);
    _selectedWeight = (clientData['weight'] ?? 70.0).toDouble();
    _displayWeight = myWeightUnit == 'kg' ? _selectedWeight : unitPreferences.kgToLbs(_selectedWeight);

    // Set fitness level
    _currentFitnessLevel = clientData['currentFitnessLevel'] ?? clientData['fitnessLevel'] ?? 'Level 1';

    // Set other form fields
    _goalsController.text = clientData['goals'] ?? '';
    _medicalHistoryController.text = clientData['medicalHistory'] ?? '';
    _injuriesController.text = clientData['injuries'] ?? '';
    _fitnessHistoryController.text = clientData['fitnessHistory'] ?? '';
    _dietaryHabitsController.text = clientData['dietaryHabits'] ?? '';
    _exercisePreferencesController.text = clientData['exercisePreferences'] ?? '';

    // Set avatar if exists
    _selectedAvatarPath = clientData['clientProfileImageUrl'];

    // Handle available hours if they exist
    if (clientData['availableHours'] != null) {
      try {
        final availableHours = clientData['availableHours']['availableHours'] as Map<String, dynamic>;
        _availableHours = availableHours.map((key, value) {
          final List<TimeRange> ranges = (value as List).map((range) {
        ...
        // Update availability controller text
        _availabilityController.text = _availableHours.entries
            .where((entry) => entry.value.isNotEmpty)
            .map((entry) =>
                '${entry.key}: ${entry.value.map((range) => '${range.start.format(context)}-${range.end.format(context)}').join(', ')}')
            .join('\n');
      } catch (e) {
        debugPrint('Error parsing available hours: $e');
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: theme.scaffoldBackgroundColor,
      appBar: _buildAppBar(),
      body: BlocConsumer<EditClientBloc, EditClientState>(
      ...
      child: Column(
        ...
        children: [
          ...
          CustomMeasurePicker(
            ...
            units: const ['cm', 'ft'],
            initialValue: heightUnit == 'cm' ? _selectedHeight : _displayHeight,
          ...
          CustomMeasurePicker(
            ...
            units: const ['kg', 'lbs'],
            initialValue: weightUnit == 'kg' ? _selectedWeight : _displayWeight,
            onChanged: (value, unit) {
              setState(() {
                weightUnit = unit;
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      context.read<EditClientBloc>().add(
            EditClient(
              data: clientData,
              isAppUser: widget.isAppUser,
              isClientInfoEnteredByTrainer: widget.clientInfoEnteredByTrainer != null ? true : false,
